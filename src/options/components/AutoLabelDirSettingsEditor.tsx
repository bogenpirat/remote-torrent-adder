...
function AutoLabelDirSettingsEditor({ value, onChange, showLabel, showDir, labels, dirs }: AutoLabelDirSettingsEditorProps) {
  ...
  return (
    <div style={{ marginBottom: 20, border: "1px solid #b7c9a7", borderRadius: 10, padding: 16, background: "#f7faf7" }}>
      <div style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
        <span style={{ fontWeight: 600, fontSize: 16, marginRight: 12 }}>Auto Label/Dir Settings</span>
        <button onClick={handleAdd} style={{ background: "#228B22", color: "#fff", border: "none", borderRadius: 8, padding: "6px 16px", fontWeight: 500, cursor: "pointer" }}>Add Rule</button>
      </div>
      {value.length === 0 && <div style={{ color: "#888", marginBottom: 8 }}>No rules defined yet.</div>}
      {value.map((setting, idx) => (
        <div key={idx} style={{ marginBottom: 18, padding: 12, border: "1px solid #b7c9a7", borderRadius: 8, background: "#fff", position: "relative" }}>
          <button
              ...
              {showLabel && (
                <div style={{ marginBottom: 10 }}>
                  ...
                  <select
                    ...
                    style={{ fontSize: 15, borderRadius: 8, padding: "4px 10px", border: "1px solid #b7c9a7", minWidth: 120 }}>
